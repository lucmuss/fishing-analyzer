...
from fishing_analyzer.utils import get_layout_dict, is_valid_fish_frame
...
def make_html_location(path_list: list[str] | None = None, filename: str = "") -> str:
            ...
            for attribute in fish_model.plotable_attributes:
                ...
                if has_valid_data_values(cleaned_attribute_values):
                    ...
                    data: list[go.Histogram] = [
                        go.Histogram(x=cleaned_attribute_values, name=attribute_name)
                    ]
                    ...
                    file_path: str = make_html_location(
                        path_list=["diagrams", "separate_histograms", fish_type], filename=attribute
                    )
...
def store_histograms_summary(fish_model: Any) -> None:
    ...
    for fish_type in config.FISH_TYPES:
        ...
        if is_valid_fish_frame(fish_frame):
            subplots_titles: list[str] = [
                utils.attribute_to_name(attribute) for attribute in fish_model.plotable_attributes
            ]

            number_attributes: int = len(fish_model.plotable_attributes)

            figure = plotly.tools.make_subplots(
                rows=number_attributes, cols=1, subplot_titles=subplots_titles
            )
            ...
            layout_dict: dict[str, Any] = get_layout_dict(
                title=fish_type, height=final_height, width=final_width
            )
            ...
            for index, attribute in enumerate(fish_model.plotable_attributes):
                ...
                if has_valid_data_values(cleaned_attribute_values):
                    ...
                    trace: go.Histogram = go.Histogram(
                        x=cleaned_attribute_values, name=attribute_to_name
                    )

                    figure.add_trace(trace, row=counter, col=1)

                    figure["layout"][f"xaxis{counter}"].update(title="Werte")

                    figure["layout"][f"yaxis{counter}"].update(title="Anzahl")

            file_path: str = make_html_location(
                path_list=["diagrams", "summary_histograms"], filename=fish_type
            )
    ...
    for fish_type in config.FISH_TYPES:
        ...
        if is_valid_fish_frame(fish_frame):
            ...
            figure = ff.create_distplot(
                hist_data=attribute_value_list,
            ...
            file_path: str = make_html_location(
                path_list=["diagrams", "summary_distribution"], filename=fish_type
            )
        ...
        if is_valid_fish_frame(fish_frame):
            for attribute in fish_model.plotable_attributes:
                ...
                if has_valid_data_values(cleaned_attribute_values):
                    ...
                    figure = ff.create_distplot(
                        hist_data=attribute_value_list, group_labels=attribute_list
                    )

                    layout_dict: dict[str, Any] = get_layout_dict(
                        title=plot_title, y_title="Dichte"
                    )

                    figure["layout"].update(layout_dict)

                    file_path: str = make_html_location(
                        path_list=["diagrams", "separate_distribution", fish_type],
                        filename=attribute,
                    )
