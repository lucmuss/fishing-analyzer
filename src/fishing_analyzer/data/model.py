...
from fishing_analyzer.data.environment.precipitation_amount import PrecipitationAmount
from fishing_analyzer.data.environment.precipitation_amount_day import PrecipitationAmountDay
from fishing_analyzer.data.environment.record_date_hour import RecordDateHour
from fishing_analyzer.data.environment.relative_humidity import RelativeHumidity
from fishing_analyzer.data.environment.sun_minutes import SunMinutes
from fishing_analyzer.data.environment.sun_minutes_day import SunMinutesDay
from fishing_analyzer.data.environment.water_temperature import WaterTemperature
from fishing_analyzer.data.environment.wind_direction import WindDirection
from fishing_analyzer.data.environment.wind_strength import WindStrength
from fishing_analyzer.data.fish.catch_date import CatchDate
from fishing_analyzer.data.fish.catch_hour import CatchHour
from fishing_analyzer.data.fish.catch_month import CatchMonth
from fishing_analyzer.data.fish.fish_type import FishType
...
class DataFrameModel:
    def __init__(self, full_base_model):
        ...
        self.data_frame = pd.DataFrame(data_set)
...
class StatisticModel:
    ...
    def __process_year_dict(self):
        return_dict = {year: self.get_frame_by_year(year) for year in config.YEAR_RANGE}
...
class ModelFactory(metaclass=Singleton):
    ...
    def full_base_model(self):
        if not self._full_base_model:
            self._full_base_model = FullBaseModel(
                environment_model=self.environment_base_model, fish_model=self.fish_base_model
            )
