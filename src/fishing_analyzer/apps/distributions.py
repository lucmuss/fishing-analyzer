...
attribute_options: list[dict[str, str]] = generate_attribute_options(fish_model=fish_frame_model)
...
def generate_distribution(
    x_values: list[float] = default_x_values,
    name: str = default_name,
    attribute_name: str = default_attribute,
    title: str = default_title,
) -> dict:
    """Generiert eine Plotly-Verteilungsfigur basierend auf den Eingabedaten."""
    attribute_list: list[str] = [name]  # Hier wurde '' zu name geändert, da es sonst leer wäre
    attribute_value_list: list[list[float]] = [x_values]
    colors: list[tuple[int, int, int]] = [config.ATTRIBUTE_COLOR_DICT[attribute_name]]

    figure: dict = ff.create_distplot(
        hist_data=attribute_value_list, group_labels=attribute_list, bin_size=0.5, colors=colors
    )

    layout_dict: dict = utils.get_layout_dict(y_title="Dichte", title=title)
...
layout = html.Div(
    children=[
        dcc.Graph(id="fish_distributions", figure=generate_distribution()),
        html.Form(
            children=[
                html.Div(
